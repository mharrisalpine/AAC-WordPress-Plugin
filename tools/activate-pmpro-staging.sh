...
ACTIVATE_RESPONSE="$TMP/activate.html"
curl -s -L -b "$CJ" -c "$CJ" "$BASE_URL/wp-admin/$ACTIVATE_REL" > "$ACTIVATE_RESPONSE"

grep -n "Plugin activated\|activated\|Paid Memberships Pro" "$ACTIVATE_RESPONSE" | sed -n '1,40p'
