import { getFakeMerchandiseProduct, getFakeMerchandiseProducts } from '@/lib/fakeMerchandise';

const ECOMMERCE_API_URL = "https://api-ecommerce.hostinger.com";
const ECOMMERCE_STORE_ID = "store_01K7DZ8FDE70F6X9YTBG4H5X1P";
...
export async function getProducts({ids, offset, limit, order, sort_by, is_hidden, to_date} = {}) {
	...
	try {
		...
		const mergedProducts = [...remoteProducts, ...getFakeMerchandiseProducts()];
		return {
		...
		const fallbackProducts = getFakeMerchandiseProducts();
		return {
...
export async function getProduct(id, {field} = {}) {
	const fakeProduct = getFakeMerchandiseProduct(id);
	if (fakeProduct) {
		return fakeProduct;
	}
...
export async function getProductQuantities({fields, product_ids}) {
	const fakeProducts = product_ids
		.map((id) => getFakeMerchandiseProduct(id))
		.filter(Boolean);

	if (fakeProducts.length === product_ids.length) {
		return {
			variants: fakeProducts.flatMap((product) =>
